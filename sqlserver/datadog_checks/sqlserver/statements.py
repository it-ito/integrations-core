...
with qstats as (
    select query_hash, query_plan_hash, last_execution_time, last_elapsed_time,
            plan_handle AS plan_handle_orig,
            statement_start_offset AS statement_start_offset_orig,
            statement_end_offset AS statement_end_offset_orig,
/*            CONCAT(
            ...
            'aaaa' as plan_handle_and_offsets,
...
qstats_aggr as (
    select query_hash, query_plan_hash, CAST(S.dbid as int) as dbid,
       ...
       max(plan_handle_orig) as plan_handle_orig,
       max(statement_start_offset_orig) as statement_start_offset_orig,
       max(statement_end_offset_orig) as statement_end_offset_orig,
...
--    convert(int,substring(plan_handle_and_offsets, second_comma_index + 1, LEN(plan_handle_and_offsets) - second_comma_index)) as statement_end_offset,
    * from qstats_aggr
--	outer apply (select CHARINDEX(',', plan_handle_and_offsets , 1) as first_comma_index) as comma_position1
--	outer apply (select CHARINDEX(',', plan_handle_and_offsets , first_comma_index + 1)  as second_comma_index) as comma_position2
...
/*    SUBSTRING(text, (statement_start_offset / 2) + 1,
    ...
    statement_start_offset_orig as statement_start_offset,
    statement_end_offset_orig as statement_end_offset,
    0 as first_comma_index,
    0 as second_comma_index,
    plan_handle_orig as plan_handle,
    qt.text,
    encrypted as is_encrypted,
    * from qstats_aggr_split
    cross apply sys.dm_exec_sql_text(plan_handle_orig) qt
"""
